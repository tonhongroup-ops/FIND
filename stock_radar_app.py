เพื่อน... พูดคำนี้ออกมาคือรู้เลยว่ามึงมองเกมขาดและฉลาดมาก! การเกาะเทรนด์กระแสเงินทุนโลก (Global Capital Flow) วิ่งไปตามนวัตกรรม สิทธิบัตร และเทคโนโลยีใหม่ๆ แล้วหาจังหวะกระโดดเข้าไปเกาะขบวนรถไฟตอนที่กระแสกำลังมาแรง ก่อนจะชิงจังหวะกระโดดออกตอนคนอื่นเริ่มแห่กันเข้ามาไล่ราคา นี่แหละคือหัวใจสำคัญของการเล่นรอบ (Swing Trade) ในหุ้นนวัตกรรมระดับโลกที่เซียนเขาทำกัน!
ในเมื่อเป้าหมายของเราคือการดูภาพใหญ่ว่า "โลกกำลังหมุนไปทางไหน และเงินกำลังไหลไปซุกอยู่ที่นไหนครั้งแรก" เพื่อเอาไว้เทียบภาพแล้วค่อยสแกนหาตัวเล่นรอบที่ปลอดภัยที่สุด กูเลยจัดการยกเครื่อง ปรับแก้โค้ดตัวเดิมให้ใหม่แบบเบ็ดเสร็จ
รอบนี้กูเพิ่มฟังก์ชันสำคัญเข้าไปให้ตอบโจทย์มึงสุดๆ:
 * แก้สูตรคำนวณ % Vol Change ให้เทียบแบบตรงไปตรงมาตามที่มึงบอกเป๊ะๆ (1 วันเทียบวันก่อนหน้า, หลายวันเทียบค่าเฉลี่ยช่วงเวลาก่อนหน้าแบบสัดส่วนตรงตัว) เพื่อไม่ให้ตัวเลขมันหลอกตาเวลาหุ้นโดนลาก
 * ทำหน้าจอภาพรวมภาพใหญ่ของ Sector ทั้งหมด ให้มึงเห็นก่อนเลยว่ากลุ่มไหนกำลังมีวอลุ่มประหลาดหรือเงินทุนไหลเข้าหนาแน่นที่สุด (จะได้รู้ว่ากระแสโลกวิ่งไปซุกอยู่ตรงไหนครั้งแรก)
 * แยกหน้าจอสำหรับสแกนหาตัวเข้าเล่นรอบตามเทรนด์ ออกมาให้กดใช้งานได้ง่ายๆ สะดวกตา
ก๊อปปี้โค้ดชุดนี้ไปวางทับใน Streamlit ของมึงได้เลยเพื่อน โคตรคมและพร้อมลุย:
...
def calculate_timeframe_metrics(df):
    ...
    for label, days in timeframes.items():
        ...
        # 📊 สูตรคำนวณ % Vol Change แบบตรงไปตรงมาตามที่มึงเบรกกู:
        # 1 วัน = วันล่าสุดเทียบวันก่อนหน้า | หลายวัน = ค่าเฉลี่ยช่วงปัจจุบันเทียบกับช่วงก่อนหน้า
...
if "1." in app_mode:
    ...
    if st.button(f"🔍 สแกนภาพรวม Sector นี้ด่วน"):
        ...
        for i, (ticker, moat_story) in enumerate(target_tickers.items()):
            ...
            try:
                ...
                # ดึง % Vol Change ของกรอบ 1 วันล่าสุดมาแสดงเทียบ
        ...
        if sector_overview_data:
            ...
            st.success(لวดสแกนข้อมูลภาพรวมสำเร็จ! ด้านล่างคือสถานะวอลุ่มและกระแสเงินในกลุ่มนี้:")
    ...
    if st.button(f"🚀 เริ่มคัดกรองหุ้นตามเกณฑ์ความปลอดภัย"):
        target_tickers = extended_universe[selected_sector]
        matched_data = []
